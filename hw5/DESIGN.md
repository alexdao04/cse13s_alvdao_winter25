...
# Introduction:
For this assignment, we're going to be creating the famous word game "Wordle". 
As the README.md stated, most of the game has been built for us already. 
That being said, the portion that we are going to write today is a series of functions in "wordle_lib.c". Completing this portion should give us a playable version of the game.

There's several ways that I can think of solving this homework. One way that my computational linguist dad suggested was regular expression. Very cool, very linguistics-related. It would be a really cool way of implementing this.
We could also probably store the pointers from the wordlist as elements of the array which then get directly accessed instead of manually indexing each one in the array.
